...
create
extension if not exists "pgcrypto" with schema demo;
...
-- experimental
create
or replace function uri_alias() returns varchar as
$$
declare
...
begin
select
into randstr string_agg(substr(dict, ceil(random() * 60)::integer, 1), '')
...
create
or replace function trigger_org_uri() returns trigger as
$$
declare
found bool = false;
begin
...
select
into found exists (select 1 from demo.org where path_uri = new.path_uri);
END LOOP;
return new;
end;
$$
...
create
or replace function trigger_repo_uri() returns trigger as
$$
declare
found bool = false;
begin
...
found
        loop
            new.path_uri =  demo.uri_alias();
select
into found exists (select 1 from demo.repository where path_uri = new.path_uri);
END LOOP;
return new;
end;
$$
...
create
or replace function trigger_folder_uri() returns trigger as
$$
declare
found bool = false;
begin
...
found
        loop
            new.path_uri =  demo.uri_alias();
select
into found exists (select 1 from demo.folder where path_uri = new.path_uri);
END LOOP;
return new;
end;
$$
...
create
or replace function trigger_project_uri() returns trigger as
$$
declare
found bool = false;
begin
...
found
        loop
            new.path_uri =  demo.uri_alias();
select
into found exists (select 1 from demo.project where path_uri = new.path_uri);
END LOOP;
return new;
end;
$$
...
create trigger org_insert
    before insert
    on demo.org
    for each row execute procedure demo.trigger_org_uri();
create trigger folder_insert
    before insert
    on demo.folder
    for each row execute procedure demo.trigger_folder_uri();
create trigger project_insert
    before insert
    on demo.project
    for each row execute procedure demo.trigger_project_uri();
create trigger repo_insert
    before insert
    on demo.repository
    for each row execute procedure demo.trigger_repo_uri();
